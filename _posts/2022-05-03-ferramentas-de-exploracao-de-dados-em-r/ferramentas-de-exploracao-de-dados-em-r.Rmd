...
description: |
  Neste post pretendo mostrar quais as principais ferramentas que usamos ao analisar dados usando a linguagem R e o tidyverse.
author:
...
[
Neste post vou falar sobre EDA.
...
No último post, falamos sobre histogramas e da importância de compreender a frequência nos seus dados. Hoje vamos fazer mais algumas análises exploratórias dos dados de um data set do `tidyverse`, portanto devemos carregar o pacote:
...
As perguntas são ferramentas que te ajudam muito, pois guiam a exploração. E como temos  sempre melhorar nossas ferramentas, ao longo da exploração vamos transformando nossas perguntas para que elas sejam mais precisas e nos ajudem a desenvolver maior entendimento dos dados. Essa parte é realmente sobre curiosidade e criatividade.
...
>1.Quais tipos de variação ocorrem dentro das minhas variáveis?
>2.Quais tipos de covariação ocorrem entre minhas variáveis?

Compreender esses conceitos pode ser um pouco complicado no começo, no mais o objetivo é fazer várias perguntas e ir melhorando-as com o tempo.
...
>Variação é a tendência dos valores de uma variável mudar a cada medida.
...
>Como a altura das pessoas, por exemplo. A cada medida que tomamos de uma pessoa ao longo de sua vida, obteremos uma resposta diferente.
>Cada variável possui seu próprio padrão de variação, o que pode revelar muitas informações para a análise.
...
Uma boa maneira de visualizar a distribuição de variáveis categóricas (que classificam) é por meio do gráfico de barras:
...
Porém nosso gráfico ainda tem problemas em sua visualização, primeiro porque está desordenado e isso dificulta as comparações, segundo pois os nomes se sobrepõem, podemos arrumar isso usando a função `fct_infreq` do pacote `forcats` (um ótimo pacote para lidar com fatores no R, já embutido no `tidyverse`, e também um anagrama para `factors` ;D):
